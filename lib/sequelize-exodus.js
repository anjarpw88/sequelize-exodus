...
SequelizeExodus.prototype.generateModelDirectlyFromDirectory = async function(directory, option){
  ...
  var models = {};
  await Object.keys(structure.tables).forEachAsync(async (key)=>{
  ...
  if(option.force && (option.forceForeignKeys || option.forceUniqueKeys)){
    console.log("ADDING Unique Keys");
    var queryRunner = QueryAction.getQueryActionRunner(this.sequelizeObject);
    await Object.keys(structure.tables).forEachAsync(async (tableName)=>{
      var table= structure.tables[tableName];
      if (table.uniqueKeys) {
        await Object.keys(table.uniqueKeys).forEachAsync(async (ukName)=>{
          try{
            console.log("\tUK",tableName, ukName, table.uniqueKeys[ukName])
            await queryRunner.addUniqueKeys(tableName, ukName, table.uniqueKeys[ukName]);
          }catch(e){
            console.log(e);
          }
        });
  ...
  if(option.force && option.forceForeignKeys){
    ...
    await Object.keys(structure.tables).forEachAsync(async (tableName)=>{
      ...
      if (table.foreignKeys) {
        await Object.keys(table.foreignKeys).forEachAsync(async (fkName)=>{
          try{
            console.log("\tFK",tableName, fkName, table.foreignKeys[fkName].fromColumns);
