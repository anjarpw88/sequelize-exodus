...
const sanitizeConstraints=function(constraints){
...
function generateTableStructure(tableName, tableDescription, foreignKeys, constraints){
    ...
    Object.keys(tableDescription).forEach(columnName=>{
      var columnDescription=tableDescription[columnName];
      columnDescription.type = SequelizeExodus.convertToDataTypes(columnDescription);
      var column = new Column(columnName, columnDescription);
      columns[columnName]=column;
    });
    var foreignKeyObjects = sanitizeForeignKeys(foreignKeys);

    var constraintObjects = sanitizeConstraints(constraints);

    var tableModel = new TableModel(tableName, columns, foreignKeyObjects, constraintObjects);
    return tableModel;
}
...
SequelizeExodus.prototype.importFromDb=async function(){
  ...
  await tableNames.awaitAll(async tableName=>{
    ...
    virtualStructure.addTable(tableName, tableStructure);
  });
...
SequelizeExodus.prototype.saveVirtualStructureFromImport=function(dir, virtualStructure){
  var fs = require('fs');
  ...
  return new Promise((resolve,reject)=>{
    ...
    Object.keys(virtualStructure.tables).forEach((tableName)=>{
      filePath = path.join(dir, tableName);
      var tableModel= virtualStructure.tables[tableName];
      var composedString = fileComposer.compose(tableModel);
