...
const compareTwoConstraintSets=function(newTable, oldTable){
  ...
  Object.keys(oldTable.constraints).forEach((key)=>{
    if(!newTable.constraints[key] && oldTable.constraints[key].toUpperCase()!="CHECK"){
...
const compareTwoColumns = function(newCol, oldCol){
    ...
    if(oldCol.defaultValue != newCol.defaultValue){
      differences.newColumn.defaultValue = newCol.defaultValue;
      differences.oldColumn.defaultValue = oldCol.defaultValue;
      differences.isDifferent = true;
    }
    ...
    console.log(oldCol._getType(Sequelize).toString());
    console.log(newCol._getType(Sequelize).toString());
    console.log();
    console.log();
...
const findForeignKeyInfosImpactedByRemovedTablesAndRemovedColumns = function(oldTables, differences){
  ...
  var temp = [];
  toBeRemovedForeignKeyInfoArray.forEach((item)=>{
  ...
  */
  Object.keys(differences.toModifyPairs).forEach((tableName) => {
    ...
    Object.keys(set.toRemoveConstraints).forEach((constraintName)=>{
      var action = (q) => q.removeConstraint(tableName, constraintName, set.toRemoveConstraints[constraintName]);
      actions.push(action);
    });
    ...
    if(set.oldPrimaryKeys != set.newPrimaryKeys){
      var table = oldStructure.tables[tableName];
      Object.keys(table.constraints).forEach((key)=>{
        if(table.constraints[key]==="PRIMARY KEY"){
          var action = (q) => q.removePrimaryKeys(tableName, key, set.oldPrimaryKeys);
          actions.push(action);
        }
      })
    };
  ...
  */
  Object.keys(differences.toModifyPairs).forEach((tableName) => {
    ...
    Object.keys(toModifyPairs).forEach((columnName)=>{
      ...
      var action = (q) => q.modifyColumn(tableName, columnName, columnPair.newColumn, columnPair.oldColumn);
      actions.push(action);
    });
  });
  /*
