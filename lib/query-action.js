...
var getQueryActionRunner = function(sequelize){
  ...
  return {
    ...
    addUniqueKeys: async function (tableName, constraintName, uniqueKeys){
      await methodSet.addUniqueKeys(tableName, constraintName, uniqueKeys);
    },
...
var queryActionTextComposer = {
  ...
  addUniqueKeys: function(tableName,constraintName, uniqueKeys){
    var template = "QueryActionRunner.addUniqueKeys(tableName,constraintName, uniqueKeys)";
    template = template.replace("tableName", q(tableName));
    template = template.replace("constraintName", q(constraintName));
    template = template.replace("uniqueKeys", val(uniqueKeys));
    return template;
  },
...
var pairedQueryActionComposer = {
  ...
  addUniqueKeys:function(tableName, constraintName, uniqueKeys){
    return {
      up:[
        queryActionTextComposer.addUniqueKeys(tableName, constraintName, uniqueKeys)
      ],
      down:[
